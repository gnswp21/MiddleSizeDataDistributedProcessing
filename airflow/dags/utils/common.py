...
def modify_job_run_json(tuning_id, cluster_name, job_run_id):
    # JSON 튜닝 컨피그 파일 로드
    tuning_file_path = f'/opt/airflow/config/{tuning_id}.json'
    with open(tuning_file_path, 'r') as file:
        tuning_config = json.load(file)

    entryPointArguments = tuning_config[{cluster_name}][{job_run_id}]['entryPointArguments']
    sparkSubmitParameters = ('--py-files local:///etl/dependency_packages.zip ' +
                             tuning_config[{cluster_name}][{job_run_id}]['sparkSubmitParameters'])
    ...
    # 수정된 JSON 파일 저장
    tmp_run_job_file_path = f'/tmp/{cluster_name}-{job_run_id}.json'
